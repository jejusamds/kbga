...
if(!in_array($mode,$allowed,true)) safe_json(['result'=>'error','msg'=>'잘못된 요청입니다.']);

$filtered = array_map(function($v){ return SQL_Injection(RemoveXSS($v)); }, $_POST);

switch($mode){
    case 'send_code_id':
        if(empty($filtered['email'])) safe_json(['result'=>'error','msg'=>'이메일을 입력해 주세요.']);
        $row = $db->row("SELECT f_user_id FROM df_site_member WHERE f_email = :e", ['e'=>$filtered['email']]);
        if(!$row) safe_json(['result'=>'error','msg'=>'입력하신 정보가 일치하는 회원이 없습니다.']);
        ...
        if($res!==true) safe_json(['result'=>'error','msg'=>'이메일 발송 실패']);
        safe_json(['result'=>'ok','msg'=>'인증번호가 발송되었습니다.']);
        break;
    case 'verify_code_id':
        if(empty($filtered['code'])) safe_json(['result'=>'error','msg'=>'인증번호를 입력해 주세요.']);
