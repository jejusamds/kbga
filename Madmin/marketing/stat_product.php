        ...
        <div class="panel">
            <form name="frm" action="<?= $PHP_SELF ?>" method="get">
                <table class="table noMargin" cellpadding="0" cellspacing="0">
                    ...
                    <tbody>
                        <tr>
                            <td height="26" align="right" style="padding-left:5px">검색 년월</td>
                            <td class="comALeft" style="padding-left:5px">
                                <select name="yval" class="form-control" style="width:auto;" onChange="this.form.submit();">
                                    <? for ($i = 2017; $i <= date("Y"); $i++) { ?>
                                        <option value="<?= $i ?>" <? if ($yval == $i) { ?>selected<? } ?>><?= $i ?> 년</option>
                                    <? } ?>
                                </select>
                                <select name="mval" class="form-control" style="width:auto;" onChange="this.form.submit();">
                                    <option value="All">:: 월 ::</option>
                                    <? for ($i = 1; $i <= 12; $i++) { ?>
                                        <option value="<?= str_pad($i, 2, "0", STR_PAD_LEFT) ?>" <? if ($mval == str_pad($i, 2, "0", STR_PAD_LEFT)) { ?>selected<? } ?>><?= str_pad($i, 2, "0", STR_PAD_LEFT) ?> 월</option>
                                    <? } ?>
                                </select>
                                ...
                                <button class="btn btn-info btn-sm" type="button" onclick="location.href='stat_product_excel.php?yval=<?= $yval ?>&mval=<?= $mval ?>&order_fg=<?= $order_fg ?>';">엑셀 다운로드</button>
                            </td>
                        </tr>
                    </tbody>
                </table>
