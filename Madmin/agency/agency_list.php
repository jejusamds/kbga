...
if($total > 0){
    ...
    $sql = "SELECT * FROM {$this_table} WHERE f_type=:type ORDER BY f_order ASC LIMIT {$offset}, {$page_set}";
            ...
            <table class="table" cellpadding="0" cellspacing="0">
                ...
                <?php if($total>0): ?>
                    ...
                    <tr>
                        ...
                        <td class="comALeft">
                            <a href="agency_input.php?mode=update&idx=<?=$row['idx']?>&page=<?=$page?>&<?=$param?>"><?=htmlspecialchars($row['f_name'],ENT_QUOTES)?></a>
                        </td>
                        <td>
                            <a href="agency_save.php?mode=prior&posi=up&idx=<?=$row['idx']?>&page=<?=$page?>&<?=$param?>" class="btn btn-default btn-xs">▲</a>
                            <a href="agency_save.php?mode=prior&posi=down&idx=<?=$row['idx']?>&page=<?=$page?>&<?=$param?>" class="btn btn-default btn-xs">▼</a>
                        </td>
                        <td><?=substr($row['wdate'],0,10)?></td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="5" height="50" class="comACenter">등록된 데이터가 없습니다.</td></tr>
                <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    <div class="box comMTop20 comMBottom20" style="width:1114px;">
        <div class="comPTop20 comPBottom20">
            ...
            <div class="comFRight comARight" style="width:15%; padding-right:10px;">
                <button class="btn btn-default btn-sm" type="button" onclick="location.href='agency_input.php?page=<?=$page?>&<?=$param?>';">등록</button>
            </div>
            <div class="clear"></div>
        </div>
    </div>
