        ...
        <div class="box" style="width:978px;">
            <div class="panel">
                <table class="table orderInfo" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        <th>자격구분</th>
                        <td class="comALeft"><input type="text" name="f_type" value="<?= htmlspecialchars($row['f_type'],ENT_QUOTES) ?>" class="form-control" style="width:40%;"></td>
                    </tr>
                    <tr>
                        <th>등록번호</th>
                        <td class="comALeft"><input type="text" name="f_reg_no" value="<?= htmlspecialchars($row['f_reg_no'],ENT_QUOTES) ?>" class="form-control" style="width:40%;"></td>
                    </tr>
                    <tr>
                        <th>자격관리기관</th>
                        <td class="comALeft"><input type="text" name="f_manage_org" value="<?= htmlspecialchars($row['f_manage_org'],ENT_QUOTES) ?>" class="form-control" style="width:60%;"></td>
                    </tr>
                    <tr>
                        <th>주무부처</th>
                        <td class="comALeft"><input type="text" name="f_ministry" value="<?= htmlspecialchars($row['f_ministry'],ENT_QUOTES) ?>" class="form-control" style="width:60%;"></td>
                    </tr>
                </table>
            </div>
        </div>
        <div class="box comMTop10 comMBottom20" style="width:978px;">
            <div class="comPTop10 comPBottom10">
                <div class="comFLeft comACenter" style="width:10%;">
                    <button class="btn btn-primary btn-sm" type="button" onclick="location.href='qualification_list.php?<?= $param ?>';">목록</button>
                </div>
                <div class="comFRight comARight" style="width:85%; padding-right:20px;">
                    ...
                    <?php if($mode==='update'): ?>
                    <button class="btn btn-danger btn-sm" type="button" onclick="if(confirm('삭제하시겠습니까?'))location.href='exec.php?table=<?= $table ?>&mode=delete&selidx=<?= $idx ?>&page=<?= $page ?>';">삭제</button>
                    <?php endif; ?>
                </div>
                <div class="clear"></div>
            </div>
